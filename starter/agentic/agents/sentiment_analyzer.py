...
Return JSON format:
{
    ...
    def analyze(
        self, ticket_text: str, metadata: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if (
                quick_analysis["urgency_score"] > 0.5
                or quick_analysis["frustration_level"] > 0.5
            ):
    ...
    def _llm_sentiment_analysis(
        ...
        try:
            ...
            if metadata:
                messages.append(
                    HumanMessage(content=f"Context: {json.dumps(metadata)}")
                )
    ...
    def _enhanced_quick_analysis(
        self, text: str, quick_result: Dict, metadata: Dict = None
    ) -> Dict[str, Any]:
        ...
        priority_boost = (
            result["urgency_score"] * 0.6 + result["frustration_level"] * 0.4
        ) * 0.5
        ...
        result["escalation_recommended"] = (
            result["frustration_level"] > 0.7 or result["urgency_score"] > 0.8
        )
    ...
    def get_response_guidelines(self, sentiment_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        guidelines = {
            ...
            "priority": (
                "high" if sentiment_data.get("priority_boost", 0) > 0.3 else "normal"
            ),
            "empathy_level": (
                "high"
