    ...
    def route(self, state: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if classification and not knowledge_results and not tool_results:
                ...
                required_tools = classification.get("classification", {}).get(
                    "requires_tools", []
                )
    ...
    def _summarize_state(self, state: Dict[str, Any]) -> str:
        ...
        if classification:
            ...
            summary_parts.append(
                f"Category: {cls.get('category')}, Urgency: {cls.get('urgency')}"
            )

        knowledge_results = state.get("knowledge_results")
        if knowledge_results:
            summary_parts.append(
                f"Knowledge search: {knowledge_results.get('count', 0)} articles found, confidence: {knowledge_results.get('confidence', 0):.2f}"
            )

        tool_results = state.get("tool_results")
        if tool_results:
            summary_parts.append(
                f"Tools executed: {len(tool_results) if isinstance(tool_results, list) else 1}"
            )
