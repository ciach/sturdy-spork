...
class KnowledgeRetriever:
    ...
    def _initialize_embeddings(self):
        ...
        try:
            ...
            cache_path = os.path.join(
                os.path.dirname(DB_PATH), f"faiss_cache_{self.account_id}"
            )
            ...
            try:
                ...
                print(
                    f"✅ Initialized and cached knowledge base with {len(documents)} articles"
                )
            except Exception as e:
                print(
                    f"✅ Initialized knowledge base with {len(documents)} articles (cache save failed: {e})"
                )
    ...
    def _keyword_search(self, query: str, top_k: int = 3) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_score = (
                sum(a["relevance_score"] for a in top_articles) / len(top_articles)
                if top_articles
                else 0
            )
