...
class MemoryManager:
    ...
    def save_interaction(
        ...
        try:
            ...
            user = (
                ...
                .filter_by(account_id=self.account_id, external_user_id=user_id)
                .first()
            )
            ...
            for msg in messages:
                ...
                msg_hash = hashlib.md5(
                    f"{ticket_id}_{msg.content}_{role.value}".encode()
                ).hexdigest()[:16]
    ...
    def get_customer_history(
        self, customer_id: str, limit: int = 5, include_messages: bool = True
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            user = (
                session.query(udahub.User)
                .filter_by(account_id=self.account_id, external_user_id=customer_id)
                .first()
            )
            ...
            tickets = (
                ...
                .filter_by(account_id=self.account_id, user_id=user.user_id)
                .order_by(desc(udahub.Ticket.created_at))
                .limit(limit)
                .all()
            )
                ...
                if include_messages:
                    ...
                    if messages:
                        ticket_data["last_message"] = (
                            messages[-1].content[:100] if messages[-1].content else ""
                        )
    ...
    def get_resolved_issues(
        ...
        try:
            ...
            for ticket in tickets:
                ...
                messages = (
                    session.query(udahub.TicketMessage)
                    .filter_by(ticket_id=ticket.ticket_id, role=udahub.RoleEnum.ai)
                    .order_by(desc(udahub.TicketMessage.created_at))
                    .first()
                )
                ...
                resolved.append(
                    {
                        "ticket_id": ticket.ticket_id,
                        "customer_id": (
                            ticket.user.external_user_id if ticket.user else None
                        ),
    ...
    def get_customer_preferences(self, customer_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not user:
                return {"is_returning_customer": False, "total_interactions": 0}
            ...
            for ticket in tickets:
                ...
                if metadata:
                    ...
                    if metadata.main_issue_type:
                        categories[metadata.main_issue_type] = (
                            categories.get(metadata.main_issue_type, 0) + 1
                        )
            ...
            return {
                ...
                "last_interaction": (
                    tickets[-1].created_at.isoformat() if tickets else None
                ),
            }
    ...
    def find_similar_resolved_issues(
        ...
        try:
            ...
            for ticket in tickets:
                ...
                score = 0
                text = (
                    f"{metadata.tags or ''} {metadata.main_issue_type or ''}".lower()
                    if metadata
                    else ""
                )
                ...
                if score > 0:
                    ...
                    resolution_msg = next(
                        (m for m in reversed(messages) if m.role == udahub.RoleEnum.ai),
                        None,
                    )
                    ...
                    scored_tickets.append(
                        {
                            ...
                            "subject": metadata.tags if metadata else None,
                            "resolution": (
                                resolution_msg.content if resolution_msg else None
                            ),
