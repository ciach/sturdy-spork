...
if mcp:
    ...
    def lookup_user(user_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "subscription": subscription_result if subscription_result.get("success") else None,
            "reservations_count": reservations_result.get("count", 0) if reservations_result.get("success") else 0,
...
if mcp:
    ...
    def get_subscription_details(user_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "usage_percentage": (used_quota / monthly_quota * 100) if monthly_quota > 0 else 0,
...
if mcp:
    ...
    def diagnose_account_issue(user_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_status": "critical" if account_status.get("is_blocked") else "needs_attention" if issues else "healthy"
        }
    ...
    def generate_account_summary(user_id: str) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "account_overview": {
                "name": user_info.get("full_name") if user_info.get("success") else "Unknown",
                "email": user_info.get("email") if user_info.get("success") else "Unknown",
                "status": "blocked" if account_status.get("is_blocked") else "active",
                "created_at": user_info.get("created_at") if user_info.get("success") else None
            },
            "subscription_overview": {
                "tier": subscription.get("tier") if subscription.get("success") else "None",
                "status": subscription.get("status") if subscription.get("success") else "No subscription",
                "monthly_quota": subscription.get("monthly_quota") if subscription.get("success") else 0
            },
            "activity_overview": {
                "total_reservations": reservations.get("count", 0) if reservations.get("success") else 0,
                "active_reservations": len([r for r in reservations.get("reservations", []) if r["status"] == "reserved"]) if reservations.get("success") else 0
            },
            "quick_actions": []
        }
