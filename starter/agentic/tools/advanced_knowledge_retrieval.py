...
class AdvancedKnowledgeRetriever:
    ...
    def _initialize_chroma(self):
        ...
        try:
            ...
            try:
                ...
                print(
                    f"✅ Created and populated ChromaDB collection: {collection_name}"
                )
    ...
    def get_similar_articles(self, article_id: str, top_k: int = 3) -> Dict[str, Any]:
        ...
        try:
            ...
            article = (
                session.query(udahub.Knowledge).filter_by(article_id=article_id).first()
            )
            ...
            if results.get("success"):
                results["articles"] = [
                    a for a in results["articles"] if a["article_id"] != article_id
                ][:top_k]
            ...
            return {"success": False, "error": f"Similar articles error: {str(e)}"}
...
def search_knowledge_advanced(
    query: str, top_k: int = 3, category: Optional[str] = None, use_hybrid: bool = True
) -> Dict[str, Any]:
    """
    ...
    return retriever.search(
        query, top_k=top_k, filter_category=category, use_hybrid=use_hybrid
    )
